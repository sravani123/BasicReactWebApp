...
class Home extends React.Component {
  ...
  handleSelectChange =  event => {  //FUTURE: combine the handlers  (or split out question types to sub-components)
   ...
   handleSubmit = (e) =>{
    ...
    axios({
        ...
        data: {
            name: name,   
            email: email,  
            message: `You have got a service request from the name : ${name} \n on Date: ${selectedDate.getDate()} /  ${selectedDate.getMonth()}  \n Time: ${selectedTime.getHours()} : ${selectedTime.getMinutes()}  email: ${email} \n  Contact details: ${mobileNumber} \n Address: ${addressOne} , \n ${addressTwo}, \n ${landMark}`
        }
    }).then((response)=>{
      console.log("============"+name);
  ...
  render() {
    const {selectedDate,selectedTime, serviceRequest} = this.state;
    const { name, email, mobileNumber,addressOne , addressTwo, landMark, selectedService  } = serviceRequest;
    const values =   { name, email, mobileNumber,addressOne , addressTwo, landMark, selectedService };
